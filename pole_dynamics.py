...
признаки матричных пучков предсказывают приближающуюся V (AUC полюсов 0.739
против 0.685 у тайминга; Transformer с учётом порядка — 0.825). Этот модуль
...
def main():
    ...
    # ── средний по всему окну модуль доминирующего полюса (строка 1 таблицы 5) ──
    pos_win_mean = pos_traj.mean(axis=1)
    neg_win_mean = neg_traj.mean(axis=1)
    stat3, p3 = mannwhitneyu(pos_win_mean, neg_win_mean, alternative='two-sided')
    print(f"\nСредний по окну |z_max| (устойчиво повышенный модуль — строка 1 таблицы 5):")
    print(f"  Перед V:    среднее={pos_win_mean.mean():.4f}, "
          f"медиана={np.median(pos_win_mean):.4f}")
    print(f"  Перед не-V: среднее={neg_win_mean.mean():.4f}, "
          f"медиана={np.median(neg_win_mean):.4f}")
    print(f"  Тест Манна-Уитни: p-value={p3:.2e} "
          f"({'значимо' if p3 < 0.05 else 'не значимо'})")
    ...
    print("к концу окна, приближаясь к |z|=1 — это визуальное подтверждение того,")
    print("что метод матричных пучков улавливает предвестник аритмии в виде")
    print("дрейфа полюса к границе устойчивости.")
