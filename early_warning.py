...
def main():
    ...
    _tf = next(r for r in results if 'Transformer' in r['name'])
    _b1 = next(r for r in results if 'Baseline 1' in r['name'])
    _b2 = next(r for r in results if 'Baseline 2' in r['name'])
    _b3 = next(r for r in results if 'Baseline 3' in r['name'])
    print(f"MULTISEED tag=early seed={RANDOM_SEED} "
          f"auc_tf={_tf['auc']:.4f} recall={_tf['recall']:.4f} "
          f"auc_rr={_b1['auc']:.4f} auc_poles={_b2['auc']:.4f} auc_norder={_b3['auc']:.4f} "
          f"gap_vs_rr_pp={(_tf['auc']-_b1['auc'])*100:.2f}")
