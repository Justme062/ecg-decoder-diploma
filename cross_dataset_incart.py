...
def evaluate_on_incart(X, y, F):
    ...
    print("=" * 72)
    print("MIT-BIH — см. вывод train.py (внутри-датасетный тест).")
    print(f"  INCART   Базовая:  Macro F1 (N,S,V,F) = {f1_base_4:.4f}")
    print(f"  INCART   Гибрид:   Macro F1 (N,S,V,F) = {f1_hybrid_4:.4f}")
    print(f"MULTISEED tag=crossds seed={SEED} incart_base={f1_base_4:.4f} incart_hybrid={f1_hybrid_4:.4f} incart_gap_pp={(f1_hybrid_4-f1_base_4)*100:.2f}")
