...
def main():
    ...
    _mn=min(_mono_all)*100; _mx=max(_mono_all)*100; _me=sum(_mono_all)/len(_mono_all)*100
    print(f"Доля монотонно меняющихся признаков по переходам: {_mn:.0f}–{_mx:.0f}% "
          f"(в среднем {_me:.0f}%). Латентное пространство гладко лишь частично: переходы "
          "между классами прослеживаются, но монотонно меняется меньшая часть признаков.")
    print(f"MULTISEED tag=interp seed={RANDOM_SEED} mono_min={_mn:.1f} mono_max={_mx:.1f} mono_mean={_me:.1f}")
